...
class Board: 
    ...
    def do_move(self, move: move_struct): #zwraca reward i terminated
        global sum_kils, sum_deads, sum_lenght
        sum_lenght += 1
        ...
        if self.player == self.PSMART and new_poss == (0, 3):
            self.last_reward += 1000
        if self.player == self.PSMART:
            vis[new_poss[0]][new_poss[1]] += 1
    ...
    def chose_action(self): #zwraca state, move
        ...
        # wybieramy ruch
        if sample > eps_threshold: #eksploatacja
            with torch.no_grad():
                move_tensor = policy_net(state_tensor).max(1).indices.view(1, 1)
                move = move_tensor.item()

        else: #eksploracja
            move_tensor = torch.tensor([[random.choice([0, 1, 2, 3])]], device=device, dtype=torch.long)
            move = move_tensor.item()
...
def main():
    ...
    print(f"{t} smart score: {score[0]}, rand score: {score[1]}\n")
    path = f'bots/{animal_name}{trybe_arg}.pth'
    torch.save(policy_net.state_dict(), path)
    
    score = [0, 0]
    global sum_deads, sum_kils, sum_lenght
    sum_lenght, sum_kils, sum_deads = 0, 0, 0
    for i in range(9):
        for j in range(7):
            vis[i][j] = 0
    ...
    path = f'stats/{animal_name}{trybe_arg}.txt'
    with open(path, 'w') as file:
        file.write(f"{score[0]} {score[1]}\n")
        file.write(f"{sum_lenght} {sum_kils} {sum_deads}\n")
        for i in range(9):
            for j in range(7):
                file.write(f"{vis[i][j]}\n")
